...
/**
   An object of this class can binarise an element of the type EdgeHistogram as of the
   ISO/IEC JTC1/SC29/WG11/M7476 (Part 3: Visual).
*/
public class EdgeHistogramTypeBinariser extends Binariser
{
   /* names of elements */
   private static final String BINS = "BinCounts";
   /* constants */
   private static final int NO_OF_BINS = 80;
            ...
            throws SAXException
   {
      ...
      if (DEBUG) System.out.println("EdgeHistogram: startElement: "+qualified);
      ...
      else if (localName.equals(BINS))
      {
         bins = new int[NO_OF_BINS];
         currentElement = BINS;
      }
   }
   ...
   public void finish()
   {
      ...
      for (int i = 0; i < bins.length; i++)
         currentChunkWriterUsed.writeByte(bins[i], 3); //BinCounts[i]
   }
   ...
   public int decode(BitToBitDataInputStream dis, Writer w, String name, String attrs)
   {
      ...
      try
      {
         ...
         for (int i = 0; i < NO_OF_BINS; i++)
            w.write(dis.readByte(3)+" ");
         w.write("</"+ns+BINS+">\n");
         w.write("</"+name+">\n");
      }
      catch (Exception x)
      {
         System.out.println("EdgeHistogram: Unable to decode bitstream");
         x.printStackTrace();
      }
   ...
   {
      return decode(dis, w, DNS+EDGE_HISTOGRAM, null);
   }
}
